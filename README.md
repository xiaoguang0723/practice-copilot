...
- 左键与右键同时按下会按列表顺序循环切换至下一套配置。切换会取消当前请求，并清空截图队列、对话轮次与历史摘要，以新会话开始；标题栏会显示当前配置名称。

## 提示词模板

- 设置页支持创建、重命名、编辑、删除和上下调整多套提示词模板；每套模板独立保存名称和提示词内容。
- 右键长按 1 秒会按列表顺序切换到下一套提示词模板，右键快速双击仍然发送当前问题。
- “切换提示词模板”快捷键可以在设置中重新录制；默认值为右键长按 1 秒。
- 切换提示词模板会取消当前请求，并清空截图队列、面板内容、对话轮次和历史摘要，以新会话开始。
- 主面板标题栏与远端副屏会实时显示当前 API 配置名称、提示词模板名称和已截图数量。

默认快捷键：

| 快捷键 | 功能 |
| --- | --- |
| 左键快速双击 | 捕获整个主屏幕并加入当前对话轮次，最多保留 5 张待发送截图 |
| 右键快速双击 | 将输入框中的问题和当前轮截图发送给模型 |
| 中键快速双击 | 清空当前截图、对话轮次和历史摘要 |
| 按住左键滚动滚轮 | 向上或向下滚动回答内容 |
| 左键与右键同时按下 | 按配置列表顺序切换 API 配置，并开启新会话 |
| 右键长按 1 秒 | 按提示词模板列表顺序切换提示词，并开启新会话 |
| `Alt+D` | 启用或关闭鼠标穿透与不可聚焦模式；可在设置中修改 |
| 中键长按 1 秒 | 显示或隐藏悬浮窗 |
| `Alt+X` | 退出应用进程 |
| `Ctrl` + 方向键 | 以 24 像素为步长移动悬浮窗 |

Practice Copilot 显示时不会主动抢占当前前台窗口。按 `Alt+D` 开启鼠标穿透后，点击会落到后方窗口且悬浮窗不可聚焦；再次按下恢复点击和输入。按住左键滚动滚轮可以在不点击悬浮窗的情况下滚动回答内容。

设置中的键盘快捷键可以直接编辑，也可以点击“录制”后按下目标按键自动填入。点击“恢复默认按键”后，表单会恢复为默认鼠标手势和快捷键，保存后生效。默认鼠标手势包括左键双击截图、右键双击发送、中键双击清空、左键与右键同时按下切换 API 配置，以及中键长按 1 秒显示或隐藏面板。
...
- 摘要生成失败时不会删除原始历史；中键双击会同时清空截图、对话轮次和历史摘要。
- 对话和图片只保存在内存中，应用退出后不会恢复历史。

关闭悬浮窗只会隐藏窗口，应用会继续在后台运行。中键长按 1 秒重新显示窗口，使用 `Alt+X` 退出应用。
